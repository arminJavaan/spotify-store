...
export default function VerifyPhone() {
  ...
  return (
    <main className="flex items-center justify-center min-h-screen bg-dark2 px-4">
      ...
      >
        <h2 className="text-xl font-bold text-primary text-center mb-6">تأیید شماره موبایل</h2>

        {error && <p className="bg-red-100 text-red-700 border border-red-300 px-4 py-2 rounded text-sm mb-4">{error}</p>}
        {message && <p className="bg-green-100 text-green-700 border border-green-300 px-4 py-2 rounded text-sm mb-4">{message}</p>}

        <p className="text-sm text-gray-light mb-2">کد ارسال‌شده به شماره <span className="text-primary">{phone}</span> را وارد کنید</p>
        <input
          type="text"
        ...
        {resendTimer > 0 ? (
          <p className="text-center text-xs mt-4 text-gray-med">ارسال مجدد تا {resendTimer} ثانیه دیگر</p>
        ) : (
          <button
